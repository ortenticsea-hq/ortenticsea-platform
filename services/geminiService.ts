  ...
  try {
    ...
    if (!client) {
      return language === 'pidgin'
        ? "Abeg, system never ready. Try again shortly."
        : "System not configured yet. Please try again shortly.";
    }
    ...
    return (
      response.text ||
      (language === "pidgin"
        ? "Abeg, I no fit process that now."
...
//   rawApiKey !== "undefined" &&
//   rawApiKey !== "null" &&
//   rawApiKey !== "your_gemini_key_here"
//     ? rawApiKey
//     : "";
...
//     if (!client) {
//       return language === 'pidgin'
//         ? "Abeg set your Gemini API key first (VITE_GEMINI_API_KEY)."
//         : "Please set your Gemini API key first (VITE_GEMINI_API_KEY).";
//     }
//     const response = await client.models.generateContent({
